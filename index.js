...
async function run() {
  try {
    ...
    const servicePackageCollections = client
      .db("seoWebsite")
      .collection("servicePackage");
    const packageTitleCollections = client
      .db("seoWebsite")
      .collection("packagesTitle");
    ...
    const AboutUsOptionCollections = client
      .db("seoWebsite")
      .collection("AboutUsOption");
    const BannerOptionCollections = client
      .db("seoWebsite")
      .collection("Banner");
    const SpecialityOptionCollections = client
      .db("seoWebsite")
      .collection("Speciality");
    const WhyChooseOptionCollections = client
      .db("seoWebsite")
      .collection("WhyChooseOption");
    const RoadMapOptionCollections = client
      .db("seoWebsite")
      .collection("roadMap");
    const TeamOptionCollections = client.db("seoWebsite").collection("team");
    const TeamTitleOptionCollections = client
      .db("seoWebsite")
      .collection("teamTitle");
    const TestimonialOptionCollections = client
      .db("seoWebsite")
      .collection("testimonials");
    const TestimonialTitleOptionCollections = client
      .db("seoWebsite")
      .collection("testimonialsTitle");
    const FaqsOptionCollections = client.db("seoWebsite").collection("faqs");
    const FaqsTitleCollections = client
      .db("seoWebsite")
      .collection("faqsTitle");
    const FooterCollections = client.db("seoWebsite").collection("footer");
    const FooterLinkCollections = client
      .db("seoWebsite")
      .collection("footerLink");
    const sliderCollections = client.db("seoWebsite").collection("slider");
    const ContactPageCollections = client
      .db("seoWebsite")
    ...
    const TicketReplyCollections = client
      .db("seoWebsite")
      .collection("TicketReply");
    const newsLetterCollections = client
      .db("seoWebsite")
      .collection("newsLetter");
    const userCollection = client.db("seoWebsite").collection("users");
    const featurePageCollections = client
      .db("seoWebsite")
      .collection("features");
    const metaCollections = client.db("seoWebsite").collection("metaInfo");
    const CreateServicesCollections = client
      .db("seoWebsite")
      .collection("CreateServices");
    const serviceTitleCollections = client
      .db("seoWebsite")
    ...
    app.put("/edit-speciality/:id", async (req, res) => {
      ...
      const updatedDoc = {
        $set: {
          ...
          headingTitleTop: updateSpeciality.headingTitleTop,
          ...
          cardOneImg: updateSpeciality.cardOneImg,
          cardTwoImg: updateSpeciality.cardTwoImg,
          cardThreeImg: updateSpeciality.cardThreeImg, 
        },
      };
    ...
    app.put("/edit-why-choose/:id", async (req, res) => {
      ...
      const updatedDoc = {
        $set: {
          ...
          bannerDescription: updateChoose.bannerDescription,
          ...
          cardDescFour: updateChoose.cardDescFour,
          cardDescFour: updateChoose.cardDescFour,
          cardImgOne: updateChoose.cardImgOne,
          cardImgTwo: updateChoose.cardImgTwo,
          cardImgThree: updateChoose.cardImgThree,
          cardImgFour: updateChoose.cardImgFour,
        },
      };
    ...
    app.post("/testimonial-title", async (req, res) => {
      ...
      const result = await TestimonialTitleOptionCollections.insertOne(
        testimonialTitle
      );
      res.send(result);
    });
    ...
    app.get("/testimonial-title/:id", async (req, res) => {
      const id = req.params.id; // Get the ID from the URL
      const query = { _id: new ObjectId(id) }; // Filter by ID
      const testimonialTitle = await TestimonialTitleOptionCollections.findOne(
        query
      ); // Use findOne to get a single testimonial
      res.send(testimonialTitle);
    });
    ...
    app.get("/check-slug/:slug", async (req, res) => {
      ...
      try {
        ...
        const matchingPosts = await CreateServicesCollections.find(
          query
        ).toArray();
